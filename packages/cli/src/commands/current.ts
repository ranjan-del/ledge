import { buildResumePrompt } from '@ledge/core';
import type { CommandContext } from '../context.ts';
import { EXIT, NotFoundError, UsageError } from '../context.ts';
import { renderTaskMarkdown, toJson, truncateLines } from '../format.ts';
 ...
 * prints the block the SessionStart hook injects, capped at 40 lines. Exit 2 when nothing
 * matches so hook scripts can branch on it.
 */
export async function run(ctx: CommandContext): Promise<number> {
  ...
  else if (ctx.flags.context) ctx.out(truncateLines(buildResumePrompt(task)));
